SYSTEM_INSTRUCTION_FIRST_PASS = """
    ...
    [
        ...
        {"query_template_class": "TopContributors", "description": "Get top contributors to the repository, if no limit is specified send limit as 10", "required_attributes": ["repo", "owner", "limit"]},
        {"query_template_class": "CheckRunStatus", "description": "Get CI/CD check run success rate and status the ref here is the commit SHA without the #", "required_attributes": ["repo", "owner", "ref"]},
        {"query_template_class": "RepositoryActivity", "description": "Get recent repository activity (commits, issues, PRs), if no limit is specified send limit as 10", "required_attributes": ["repo", "owner", "limit"]},
        ...
        {"query_template_class": "LatestIssueAndHint", "description": "Get the latest open issue and provide hints on how to solve it - PRIORITY: Focus on this class when user asks about latest issues, newest issues, or current problems", "required_attributes": ["repo", "owner"]},
